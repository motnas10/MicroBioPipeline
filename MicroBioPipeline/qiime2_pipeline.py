...
python Executor.py ./QIIME2_pipeline.py --args import-qc \
  --manifest ./fastq_SanCamillo/fastqPaired/manifest.tsv \
  --paired \
  --demux_out paired-demux.qza \
...
python QIIME2_pipeline.py run \
  --demux ./fastq_SanCamillo/fastqPaired/results/01_demux/paired-demux.qza \
  --classifier ./Classifiers/SilvaClassifier/silva-138-99-nb-classifier.qza \
  --metadata ./fastq_SanCamillo/fastqPaired/sample-metadata.tsv \
  --paired \
  --trim_left_f 17 --trim_left_r 21 \
  --trunc_len_f 270 --trunc_len_r 210 \
...
python Executor.py ./QIIME2_pipeline.py --args run \
  --demux ./fastq_SanCamillo/fastqPaired/results/01_demux/paired-demux.qza \
  --classifier ./Classifiers/SilvaClassifier/silva-138-99-nb-classifier.qza \
  --metadata ./fastq_SanCamillo/fastqPaired/sample-metadata.tsv \
  --paired \
  ...
  --depth 20864 \
  --permanova \
  --permanova_column condition,sex,BMI,IL10,CXCL10
...
import re
import qiime2
from qiime2.plugins import demux, dada2, feature_classifier, feature_table, alignment, phylogeny, diversity, taxa
...
def main():
    parser = argparse.ArgumentParser(
        description="QIIME2 16S pipeline: manifest -> import+QC -> denoise+filter+custom diversity"
    )
