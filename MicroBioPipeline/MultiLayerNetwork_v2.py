...
# Check for igraph (CPU Spinglass)
try:
    import igraph as ig
    IGRAPH_AVAILABLE = True
except ImportError:
    IGRAPH_AVAILABLE = False
    warnings.warn("igraph not available. Some community detection methods (Spinglass) will not work.")

# Check for RAPIDS (GPU Acceleration)
try:
    import cugraph
    import cudf
    ...
    # We don't warn immediately to avoid spam on non-GPU machines, 
    # but we will raise an error if GPU methods are requested.
...
def project_to_simplex(v: np.ndarray) -> np.ndarray:
    ...
    """
    # print('[INFO] Projecting to simplex...') # Commented out to reduce verbosity during optimization
...
def detect_communities_gpu(G: nx.Graph, resolution: float = 1.0, use_leiden: bool = True) -> List[List[Any]]:
    """
    GPU-accelerated community detection using NVIDIA RAPIDS (cuGraph).
    ...
    use_leiden : bool
        If True, uses Leiden (higher quality). If False, uses Louvain.
        
    Returns
    -------
    List[List[Any]]
        List of communities, where each community is a list of node labels.
    """
    if not CUGRAPH_AVAILABLE:
        raise ImportError("NVIDIA RAPIDS (cugraph/cudf) is required for GPU acceleration.")

    if len(G) == 0:
        return []

    # 1. Convert NetworkX graph to cuGraph compatible format
    # Extract edges to pandas first
    df = nx.to_pandas_edgelist(G)
    
    if 'weight' not in df.columns:
        df['weight'] = 1.0
    else:
        # cuGraph Leiden/Louvain generally assumes positive weights for modularity.
        # If weights are negative, standard modularity maximization might behave unexpectedly 
        # unless specifically handled. For now, we pass as is.
        df['weight'] = df['weight'].astype(float)
        
    # Move to GPU memory
    gdf = cudf.DataFrame.from_pandas(df[['source', 'target', 'weight']])
    ...
    # 2. Run Algorithm
    if use_leiden:
        partition_df, _ = cugraph.leiden(G_gpu, resolution=resolution)
    else:
        partition_df, _ = cugraph.louvain(G_gpu, resolution=resolution)

    # 3. Format results back to List[List[NodeID]]
    # Un-renumber if needed to get original node labels back
    if G_gpu.renumbered:
        partition_df = G_gpu.unrenumber(partition_df, "vertex")

    # Move result back to CPU
    result_pdf = partition_df.to_pandas()
    
    # Group by partition ID
    communities_dict = result_pdf.groupby('partition')['vertex'].apply(list).to_dict()
...
class MultiplexCommunityOptimizer:
    ...
    - Detect communities using various algorithms (CPU and GPU)
    ...
    def _load_correlation_layers(
        ...
        for sheet in sheets:
            ...
            # Percolation step could be added here
            from .datasetmanagement import percolation_threshold
            _, corr_matrix = percolation_threshold(corr_matrix)
    ...
    def collapse_layers(
        ...
        nx.Graph
            Collapsed network.
        """
        # print('[INFO] Collapsing layers into single network...') # Verbose
        if weights is None:
    ...
    def detect_communities(
        ...
        **method_kwargs
        ...
        method : str, default='spinglass'
            ...
            - 'leiden_gpu': GPU-accelerated Leiden (requires cugraph)
            - 'louvain_gpu': GPU-accelerated Louvain (requires cugraph)
            - 'greedy_modularity': Greedy modularity optimization
            - 'label_propagation': Label propagation
        custom_method : Optional[Callable], default=None
            Custom community detection function.
        **method_kwargs
        ...
        """
        # print('[INFO] Detecting communities...') # Verbose
        if G is None:
        ...
        elif method in ['leiden_gpu', 'louvain_gpu']:
            return self._detect_gpu(G, method=method, **method_kwargs)
    ...
    def _detect_gpu(self, G: nx.Graph, method: str, **kwargs) -> List[List[Any]]:
        """Internal handler for GPU methods."""
        use_leiden = (method == 'leiden_gpu')
        # Filter kwargs to only pass relevant ones to detect_communities_gpu
        resolution = kwargs.get('resolution', 1.0)
        return detect_communities_gpu(G, resolution=resolution, use_leiden=use_leiden)
    ...
    def score_communities(
        ...
        # print('[INFO] Scoring communities...') # Verbose
    ...
    def objective(
        ...
        **kwargs
        ...
        # print('[INFO] Evaluating objective function...') # Verbose
        weights = project_to_simplex(weights)
        
        vals = []
        # Only use tqdm if verbose is True to avoid spamming bars inside optimization loops
        iterable = range(repeats)
        if verbose and repeats > 1:
            iterable = tqdm(iterable, total=repeats, desc="Repeats")
    ...
    def optimize(
        ...
        **optimizer_kwargs
        ...
        # Check GPU availability early if requested
        if 'gpu' in community_method and not CUGRAPH_AVAILABLE:
            raise ImportError("GPU method requested but NVIDIA RAPIDS is not available.")
